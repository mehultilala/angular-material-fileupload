...
export class MatFileUploadComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    if (queueInput) {
      ...
      this.requestBody = this.requestBody || queueInput.requestBody;
    }
  }
  ...
  public upload(): void {
    ...
    formData.set('details', JSON.stringify(this.requestBody || ''));
    this.subs.add(
      this.HttpClient.post(this.httpUrl, formData, {
        ...
        complete: () => this.uploadInProgressSubject.next(false),
      })
    );
  }
